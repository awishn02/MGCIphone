...
- (BOOL)application:(UIApplication *)application openURL:(NSURL *)url sourceApplication:(NSString *)sourceApplication annotation:(id)annotation{
	NSString *stringNoPrefix = [[url.absoluteString componentsSeparatedByString:@"://"] lastObject];
	NSMutableArray *parts = [[stringNoPrefix componentsSeparatedByString:@"/"] mutableCopy];
	[[NSUserDefaults standardUserDefaults] setObject:[parts objectAtIndex:0] forKey:@"username"];
	[[NSUserDefaults standardUserDefaults] setObject:[parts objectAtIndex:1] forKey:@"user_id"];
	return YES;
}
